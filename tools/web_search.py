...
def is_trusted_source_ai(url: str, title: str, content: str) -> dict:
    ...
    try:
        ...
        prompt = f"""以下のWeb検索結果が、中小企業の価格転嫁に関する信頼できる情報源かどうかを判定してください。
...
【判定基準】
信頼できるソース（trusted）とみなす条件：
1. 日本政府機関（○○省、○○庁など）のサイト（.go.jp）
2. 地方自治体のサイト（.lg.jp）
3. 公的な中小企業支援機関（中小機構、商工会議所、よろず支援拠点など）
4. 独立行政法人・公的金融機関
5. 大学・研究機関の公式サイト（.ac.jp）
6. 信頼できる業界団体の公式サイト

信頼できないソース（untrusted）とみなす条件：
1. 個人ブログ、アフィリエイトサイト
2. まとめサイト、キュレーションサイト
3. 広告目的のサイト
4. 情報源が不明確なサイト
