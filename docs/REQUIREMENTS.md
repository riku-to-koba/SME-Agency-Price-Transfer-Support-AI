...
**ツール直接呼び出しの例:**
- ユーザー: 「取引先の財務状況を知りたい」
  - Mode 2 Agent: `web_search`ツールを直接呼び出し
- ユーザー: 「いくら値上げすればいいか分からない」
  - Mode 2 Agent: `calculate_cost_impact`ツールを直接呼び出し
- ユーザー: 「原材料の価格推移を調べて」
  - Mode 2 Agent: `web_search`ツールを直接呼び出し
...
```
ユーザー: 「原価計算したい」
→ Mode 2 Agent: calculate_cost_impactツールを実行
...
*   **AI支援**:
    1.  `web_search`使用: ユーザーの主要原材料（鉄、樹脂、食品等）を特定し、過去3年の価格推移データをWeb検索で取得。
    2.  `web_search`使用: 地域別最低賃金の推移データを取得し、労務費上昇の根拠とする。
    3.  視覚化: `generate_chart`を使用して上昇トレンドが一目でわかる「インパクト・グラフ」を作成し、ユーザーに提示。
...
*   **AI支援**:
    ...
    2.  `calculate_cost_impact`使用: 対話形式で変動費（材料・外注）、固定費（人件費・家賃）を入力させ、配賦計算を実行。
...
*   **AI支援**:
    1.  `web_search`使用: 取引先の最新決算、ニュース、中期計画をWeb検索で調査。
...
*   **AI支援**:
    ...
    2.  `simulate_negotiation`使用: 発見した強みを言語化し、交渉時の「切り札トーク」としてシナリオに組み込む。
...
*   **AI支援**:
    1.  `calculate_cost_impact`使用: 全データを統合し、最終的な価格改定シミュレーションを実行。
...
*   **AI支援**:
    1.  `web_search`使用: 同業他社の値上げニュース、業界団体の「値上げのお願い」声明文を収集。
...
*   **AI支援**:
    1.  `web_search`使用: 取引先リストを分析。「パートナーシップ宣言あり」「業績好調」「取引依存度が低い（切りやすい）」相手をSランクに設定。
...
#### STEP 4: 交渉本番（プレゼンテーション & クロージング）
*   **目的**: 納得ずくで値上げを承諾させる
*   **ゴール**: 準備した資料とロジックを駆使し、納得ずくで値上げを承諾させる。
*   **AI支援**:
    1.  `document_generator`使用: 「根拠資料セット（グラフ、試算表、見積書）」を最終出力。
    2.  `simulate_negotiation`使用: 当日の交渉シナリオ（台本）を表示。想定問答集を手元に用意させる。
...
*   **登録データ**:
    ...
    *   公取委事例集
    *   原価計算モデル
*   **使用ツール**: `search_knowledge_base`, `calculate_cost_impact`
...
*   **保存データ**:
    ...
    *   会話履歴（24時間自動削除）
    *   アップロードファイル（見積書等）
*   **使用ツール**: `document_generator`, `generate_chart`
